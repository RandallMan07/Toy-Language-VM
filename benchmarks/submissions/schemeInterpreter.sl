...
function evalIf(ast, env) {
	...
	els = pop(ast);
	if (eval1(cond,env)){
		return eval1(ten, env);
	}
	return eval1(els,env);
}
function evalDefine(ast, env) {
	sym = pop(ast);
	def = pop(ast);
	set(env, sym.val,eval1(def,env));
}
